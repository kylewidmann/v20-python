...
class Candlestick(BaseEntity):
    ...
    def from_dict(data, ctx):
        ...
        if data.get("bid") is not None:
            data["bid"] = ctx.instrument.CandlestickData.from_dict(data["bid"], ctx)

        if data.get("ask") is not None:
            data["ask"] = ctx.instrument.CandlestickData.from_dict(data["ask"], ctx)

        if data.get("mid") is not None:
            data["mid"] = ctx.instrument.CandlestickData.from_dict(data["mid"], ctx)
...
class CandlestickData(BaseEntity):
    ...
    def from_dict(data, ctx):
        ...
        if data.get("h") is not None:
            data["h"] = ctx.convert_decimal_number(data.get("h"))

        if data.get("l") is not None:
            data["l"] = ctx.convert_decimal_number(data.get("l"))

        if data.get("c") is not None:
            data["c"] = ctx.convert_decimal_number(data.get("c"))
...
class OrderBook(BaseEntity):
    ...
    def from_dict(data, ctx):
        ...
        if data.get("bucketWidth") is not None:
            data["bucketWidth"] = ctx.convert_decimal_number(data.get("bucketWidth"))
...
class EntitySpec(object):
    ...
    def candles(self, instrument, **kwargs):
        ...
        request.set_param("smooth", kwargs.get("smooth"))

        request.set_param("includeFirst", kwargs.get("includeFirst"))

        request.set_param("dailyAlignment", kwargs.get("dailyAlignment"))

        request.set_param("alignmentTimezone", kwargs.get("alignmentTimezone"))

        request.set_param("weeklyAlignment", kwargs.get("weeklyAlignment"))
    ...
    def price(self, instrument, **kwargs):
        ...
        if str(response.status) == "200":
            if jbody.get("price") is not None:
                parsed_body["price"] = self.ctx.pricing_common.Price.from_dict(
                    jbody["price"], self.ctx
                )
    ...
    def prices(self, instrument, **kwargs):
        """
        ...
        request = Request("GET", "/v3/instruments/{instrument}/price/range")
    ...
    def order_book(self, instrument, **kwargs):
        """
        ...
        request = Request("GET", "/v3/instruments/{instrument}/orderBook")
        ...
        if str(response.status) == "200":
            if jbody.get("orderBook") is not None:
                parsed_body["orderBook"] = self.ctx.instrument.OrderBook.from_dict(
                    jbody["orderBook"], self.ctx
                )
