...
class Field(Property):
    ...
    def __str__(self):
        if self.typeClass.startswith("array"):
            return "{} = [{}]".format(self.displayName, len(self.value))
        ...
        if self.typeClass.startswith("object"):
            ...
            return "{} = {}".format(self.displayName, v)
...
class BaseEntity(object):
    ...
    def ordered_dict(self, verbose):
        ...
        for field in self.fields():
            ...
            d[key] = field.value_ordered_dict(verbose)
    ...
    def yaml(self, verbose=False):
        ...
        return yaml.safe_dump(
            self.ordered_dict(verbose=verbose), default_flow_style=False, indent=2
        ).strip()
    ...
    def dict(self):
        ...
        for field in self.fields():
            if (
                field.typeName == "primitives.DecimalNumber"
                or field.typeName == "primitives.AccountUnits"
                or field.typeName == "pricing.PriceValue"
            ):
            ...
            if other_value is None:
                raise RuntimeError(
                    "Did not expected {}.{} to be unset".format(
                        self.__class__.__name__, property.name
                    )
                )
