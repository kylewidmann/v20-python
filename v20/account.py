...
class Account(BaseEntity):
    ...
    def from_dict(data, ctx):
        ...
        if data.get("positions") is not None:
            data["positions"] = [
                ctx.position.Position.from_dict(d, ctx) for d in data.get("positions")
            ]
...
class AccountChanges(BaseEntity):
    ...
    def from_dict(data, ctx):
        ...
        if data.get("ordersCreated") is not None:
            data["ordersCreated"] = [
                ctx.order.Order.from_dict(d, ctx) for d in data.get("ordersCreated")
            ]

        if data.get("ordersCancelled") is not None:
            data["ordersCancelled"] = [
                ctx.order.Order.from_dict(d, ctx) for d in data.get("ordersCancelled")
            ]

        if data.get("ordersFilled") is not None:
            data["ordersFilled"] = [
                ctx.order.Order.from_dict(d, ctx) for d in data.get("ordersFilled")
            ]

        if data.get("ordersTriggered") is not None:
            data["ordersTriggered"] = [
                ctx.order.Order.from_dict(d, ctx) for d in data.get("ordersTriggered")
            ]
...
class EntitySpec(object):
    ...
    def get(self, accountID, **kwargs):
        ...
        request = Request("GET", "/v3/accounts/{accountID}")
        ...
        if str(response.status) == "200":
            if jbody.get("account") is not None:
                parsed_body["account"] = self.ctx.account.Account.from_dict(
                    jbody["account"], self.ctx
                )
    ...
    def summary(self, accountID, **kwargs):
        """
        ...
        request = Request("GET", "/v3/accounts/{accountID}/summary")
        ...
        if str(response.status) == "200":
            if jbody.get("account") is not None:
                parsed_body["account"] = self.ctx.account.AccountSummary.from_dict(
                    jbody["account"], self.ctx
                )
    ...
    def instruments(self, accountID, **kwargs):
        """
            ...
            if jbody.get("clientConfigureTransaction") is not None:
                parsed_body["clientConfigureTransaction"] = (
                    self.ctx.transaction.ClientConfigureTransaction.from_dict(
                        jbody["clientConfigureTransaction"], self.ctx
                    )
                )
            ...
            if jbody.get("clientConfigureRejectTransaction") is not None:
                parsed_body["clientConfigureRejectTransaction"] = (
                    self.ctx.transaction.ClientConfigureRejectTransaction.from_dict(
                        jbody["clientConfigureRejectTransaction"], self.ctx
                    )
                )
    ...
    def changes(self, accountID, **kwargs):
        """
        ...
        request = Request("GET", "/v3/accounts/{accountID}/changes")

        request.set_path_param("accountID", accountID)

        request.set_param("sinceTransactionID", kwargs.get("sinceTransactionID"))
        ...
        if str(response.status) == "200":
            if jbody.get("changes") is not None:
                parsed_body["changes"] = self.ctx.account.AccountChanges.from_dict(
                    jbody["changes"], self.ctx
                )

            if jbody.get("state") is not None:
                parsed_body["state"] = self.ctx.account.AccountChangesState.from_dict(
                    jbody["state"], self.ctx
                )
